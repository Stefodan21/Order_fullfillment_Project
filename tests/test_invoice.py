...
def test_generate_invoice_missing_required_fields():
    client = api.test_client()
    payload = {
        "customer_name": "Stefaan"  # Missing other required fields
    }
    response = client.post(
        "/invoiceGenerator",
        data=json.dumps(payload),
...
def test_generate_invoice_invalid_data_types():
    client = api.test_client()
    payload = {
        "customer_name": "Stefaan",
        "customer_address": "Jamaica",
        "business_name": "CloudSoft",
        "item_purchased": "Serverless Mastery",
        "item_price": "invalid_price",  # Should be numeric
        "item_quantity": 2,
        "status": "confirmed"
    }
    response = client.post(
